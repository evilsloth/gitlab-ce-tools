...
export class GroupsApiService {
    ...
    getProjectsOfGroupDeep(groupId: string|number, includeArchived: boolean, projectNameFilterTerm?: string): Observable<Project[]> {
        const projectsOfGroup$ = this.getProjectsOfGroup(groupId, includeArchived, projectNameFilterTerm);
        const projectsOfSubgroups$ = this.getSubgroups(groupId).pipe(
            map(subgroups => subgroups.map(sg => this.getProjectsOfGroupDeep(sg.id, includeArchived, projectNameFilterTerm))),
    ...
    getProjectsOfGroup(groupId: string|number, includeArchived: boolean, projectNameFilterTerm?: string): Observable<Project[]> {
